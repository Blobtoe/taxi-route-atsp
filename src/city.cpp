...
City::City(const int index, const std::vector<std::vector<int>>& matrix,
...
int City::get_bound() const { return lower_bound_; }

size_t City::get_path_size() const { return previous_path_.path_.size(); }

int City::get_index() const { return index_; }

Path City::get_path() const { return previous_path_; }
...
{
    reduction_matrix_.fill_passed_nodes(previous_path_.get_prev_city(), index_);
