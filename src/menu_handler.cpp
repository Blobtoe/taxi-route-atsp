...
{
    clear_term();
    std::string subtitles[] = {"Brute Force",
    ...
    while (!exit) {
        ...
        switch (choice) {
            case 1: {
                ...
                time = run_algo([&brutef]() { return brutef.run(); });
            ...
            case 2: {
                ...
                time = run_algo([&bnb]() { return bnb.best_fs(); });
            ...
            case 3: {
                display_matrix();
                auto bnb{tsp::branch_n_bound(matrix)};
                time = run_algo([&bnb]() { return bnb.dfs(); });
                std::cout << "  Time [ms] >> " << time;
                wait_for_reaction();
                break;
            }
            case 4: {
                ...
                time = run_algo([&hk]() { return hk.run(); });
                std::cout << "  Time [ms] >> " << time;
                wait_for_reaction();
                break;
            }
...
void Menu::run_all_algos()
{
    ...
    time = run_algo([&bf]() { return bf.run(); });
    std::cout << "  Time[ms] >> " << time << "\n\n";
