...
brute_force::brute_force(Adjacency_Matrix& matrix) : mat_{matrix} {}
...
{
    for (int i{0}; i < (int)perms.size() - 1; ++i) perms[i] = i;
    perms[perms.size() - 1] = 0;
}

void brute_force::permutate(std::vector<int>& perms, int begin, int end,
                            Path& p)
{
    if (begin == end)
